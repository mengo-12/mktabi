...
from app.schemas.calendar import CalendarEvent, CalendarEventUpdate, CalendarEventCreate
...
async def update_section(
    ...
    if getattr(current_user, "is_dynamic_staff", False):
        raise HTTPException(
            status_code=403, detail="غير مصرح لك بتعديل الأقسام الرئيسية."
        )
...
async def delete_section(
    ...
    if getattr(current_user, "is_dynamic_staff", False):
        raise HTTPException(
            status_code=403, detail="غير مصرح لك بحذف الأقسام الرئيسية."
        )
...
async def create_table(
    ...
    if getattr(current_user, "is_dynamic_staff", False):
        raise HTTPException(
            status_code=403, detail="غير مصرح للموظفين بإنشاء جداول جديدة."
        )
...
async def get_tables_by_section(
    ...
    result = await db.execute(
        select(CustomTable).filter(CustomTable.section_id == section_id)
    )
    ...
    if current_user.role == UserRole.ADMIN or current_user.role == UserRole.PARTNER:
        for table in tables:
    ...
    if getattr(current_user, "is_dynamic_staff", False):
        ...
        for table in tables:
            ...
            permission = permissions.get(str(table.id), "no_access")
...
async def update_table_structure(
    ...
    if getattr(current_user, "is_dynamic_staff", False):
        raise HTTPException(
            status_code=403,
            detail="غير مصرح للموظفين بتعديل هيكلية الجداول أو الأعمدة.",
        )
    ...
    db_table.columns_definition = [
        col.model_dump() for col in updated_data.columns_definition
    ]
    db_table.view_mode = updated_data.default_view
    db_table.is_staff_table = updated_data.is_staff_table
    db_table.display_column = updated_data.display_column

    if updated_data.calendar_mapping:
        db_table.calendar_mapping = updated_data.calendar_mapping.model_dump()
    else:
...
async def delete_table(
    ...
    if getattr(current_user, "is_dynamic_staff", False):
        raise HTTPException(
            status_code=403, detail="إجراء محظور. لا يمكنك حذف الجداول."
        )
...
async def update_row_cell(
    ...
    actual_data = (
        ...
        if isinstance(updated_cells, dict)
        else updated_cells
    )
...
async def get_all_tables(
    ...
    if getattr(current_user, "is_dynamic_staff", False):
        user_perms = current_user.dynamic_permissions or {}
        return [
            t for t in tables if user_perms.get(str(t.id), "no_access") != "no_access"
        ]
...
async def get_row(
    ...
    result = await db.execute(select(CustomRow).where(CustomRow.id == row_id))
    ...
    # التحقق من الصلاحيات
    check_dynamic_permission(current_user, row.table_id, required_level="read")

    return {"id": row.id, "table_id": row.table_id, "cells_data": row.cells_data or {}}
...
async def get_calendar_events(
    ...
    result = await db.execute(
        select(CustomTable).options(selectinload(CustomTable.section))
    )
    ...
    for table in tables:
        ...
        if getattr(current_user, "is_dynamic_staff", False):
            ...
            permission = permissions.get(str(fresh_table.id), "no_access")
...
async def get_calendar_event_details(
    ...
    # جلب السجل
    result = await db.execute(
        select(CustomRow).where(CustomRow.id == row_id, CustomRow.table_id == table_id)
    )
...
async def update_calendar_event(
    ...
    if end_field:
        cells[end_field] = payload.end.isoformat() if payload.end else None

    if not start_field:
        raise HTTPException(status_code=400, detail="لم يتم تحديد حقل بداية الحدث.")
...
async def create_calendar_event(
    ...
    # التحقق من صلاحية الكتابة
    check_dynamic_permission(current_user, payload.table_id, required_level="write")
    ...
    if not start_field:
        raise HTTPException(
            status_code=400, detail="لم يتم تحديد حقل تاريخ البداية في إعدادات التقويم."
        )

    if not cells.get(start_field):
        raise HTTPException(status_code=400, detail="يجب إدخال تاريخ البداية.")

    # حماية جدول الموظفين
    if table.is_staff_table:
        cells = sanitize_staff_permissions(cells, table.id)

    row = CustomRow(table_id=payload.table_id, cells_data=cells)
