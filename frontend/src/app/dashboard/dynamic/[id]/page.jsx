    ...
    return (
        <div className="md:mr-64 min-h-screen min-w-0 p-6 bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-100" dir="rtl">
            ...
            <div className="flex flex-wrap gap-2 border-b border-gray-200 dark:border-slate-800 pb-3 mb-6">
                {tables.map((tab) => (
                    <button
                        key={tab.id}
                        onClick={() => handleTableChange(tab)}
                        className={`px-4 py-2 rounded-xl text-xs font-bold transition-all ${activeTable?.id === tab.id ? 'bg-blue-600 dark:bg-blue-500 text-white shadow-md' : 'bg-white text-slate-600 hover:bg-slate-100 border-gray-200 border dark:bg-slate-900 dark:text-slate-300 dark:border-slate-800 dark:hover:bg-slate-800'}`}
                    >
                        📊 {tab.name}
                    </button>
                ))}
            ...
            {activeTable && (
                <div>
                    {/* شريط التحكم الرئيسي */}
                    <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4 mb-4 bg-white dark:bg-slate-900 shadow-sm p-4 rounded-xl border border-gray-200 dark:border-slate-800">
                        <div>
                        ...
                        <div className="flex flex-wrap items-center gap-3 w-full lg:w-auto justify-end">
                            ...
                            <div className="flex bg-slate-100 dark:bg-slate-950 p-1 rounded-lg border border-gray-200 dark:border-slate-800">
                                ...
                                <button
                                    onClick={() => setViewMode('calendar')}
                                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all whitespace-nowrap ${viewMode === 'calendar'
                                        ? 'bg-blue-600 dark:bg-blue-500 text-white shadow-lg font-black'
                            ...
                            {hasPermission('write') && (
                                <button
                                    onClick={openSettingsModal}
                                    className="bg-white border border-gray-200 hover:bg-slate-100 text-slate-600 text-xs font-bold px-3 py-2 rounded-xl flex items-center gap-1.5 dark:bg-slate-900 dark:border-slate-800 dark:hover:bg-slate-800 dark:text-slate-300"
                                >
                                    <Settings className="w-4 h-4" />
                                    هيكلة الحقول المتقدمة
                                </button>
                ...
                ${hasPermission('write')
                                        ...
                                        : 'bg-slate-200 dark:bg-slate-800 text-slate-400 dark:text-slate-500 opacity-50 cursor-not-allowed'
                                    }`}
                            >
                                <Plus className="w-4 h-4" />
                                إضافة سجل جديد
                    ...
                    {/* البحث والتصفية */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-6 bg-white dark:bg-slate-900 p-3 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm">
                        ...
                        <div className="relative">
                            ...
                            <input
                                ...
                                value={searchTerm}
                                onChange={(e) => setSearchTerm(e.target.value)}
                                className="w-full bg-white border border-gray-200 rounded-xl py-2 pr-9 pl-3 text-xs text-slate-900 placeholder-slate-500 focus:outline-none focus:border-blue-600 dark:bg-slate-950 dark:border-slate-800 dark:text-slate-100"
                            />
                        </div>

                        <div className="flex gap-2 col-span-1 md:col-span-2 items-center flex-wrap md:flex-nowrap">

                            <div className="flex items-center gap-1.5 text-slate-500 dark:text-slate-400 text-xs shrink-0">
                                <Filter className="w-3.5 h-3.5 text-blue-600 dark:text-blue-500" />
                                <span>تصفية بحسب:</span>
                            </div>
                            ...
                            <select
                                ...
                                className="bg-white border border-gray-200 dark:bg-slate-950 dark:border-slate-800 rounded-xl p-2 text-xs text-slate-700 dark:text-slate-300 focus:outline-none"
                            >
                                <option value="">إختر الحقل...</option>
                                ...
                                <div className="flex gap-1.5 items-center w-full">
                                    ...
                                    {activeFilterColumnObject.type === 'number' && (
                                        <select
                                            value={filterOperator}
                                            onChange={(e) => setFilterOperator(e.target.value)}
                                            className="bg-white border border-gray-200 dark:bg-slate-950 dark:border-slate-800 rounded-xl p-2 text-xs text-blue-600 dark:text-blue-400 font-bold"
                                        >
                                            <option value="gt">أكبر من (&gt;)</option>
                                            <option value="lt">أصغر من (&lt;)</option>
                                            <option value="eq">يساوي (=)</option>
                                    ...
                                    {activeFilterColumnObject.type === 'date' ||
                                        ...
                                        <div className="flex items-center gap-1 w-full">
                                            <input
                                                ...
                                                value={filterDateStart}
                                                onChange={(e) => setFilterDateStart(e.target.value)}
                                                className="bg-white border border-gray-200 dark:bg-slate-950 dark:border-slate-800 rounded-xl p-1.5 text-xs text-slate-700 dark:text-slate-300 max-w-[130px]"
                                            />
                                        ...
                                        <select
                                            value={filterValue}
                                            onChange={(e) => setFilterValue(e.target.value)}
                                            className="w-full bg-white border border-gray-200 dark:bg-slate-950 dark:border-slate-800 rounded-xl p-2 text-xs text-blue-600 dark:text-blue-400"
                                        >
                                            <option value="">كل الخيارات...</option>
                                        ...
                                        <input
                                            ...
                                            value={filterValue}
                                            onChange={(e) => setFilterValue(e.target.value)}
                                            className="w-full bg-white border border-gray-200 dark:bg-slate-950 dark:border-slate-800 rounded-xl p-2 text-xs text-slate-900 dark:text-slate-200"
                                        />
                            ...
                            {(searchTerm || selectedFilterColumn) && (
                                <button
                                    onClick={resetFilters}
                                    className="p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-950 dark:hover:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200 transition text-[10px] flex items-center gap-1"
                                >
                                    <RefreshCw className="w-3 h-3" />
                                    مسح
                                </button>
                    ...
                    {viewMode === 'table' && (
                        <div className="overflow-x-auto bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800">
                            <table className="w-full text-right border-collapse min-w-[700px]">
                                <thead className="bg-slate-50 dark:bg-slate-950 text-slate-500 dark:text-slate-400 text-xs font-bold border-b border-gray-200 dark:border-slate-800">
                                    <tr>
                                        {activeTable.columns_definition.map((col) => (
                                            <th key={col.id} className="p-4">{col.name}</th>
                                        ))}
                                ...
                                <tbody className="text-xs text-slate-700 dark:text-slate-300 divide-y divide-gray-200 dark:divide-slate-800/50">
                                    {filteredRows.length === 0 ? (
                                        <tr>
                                            <td colSpan={activeTable.columns_definition.length + 1} className="p-8 text-center text-slate-500 italic">
                                                لا توجد سجلات مطابقة لخيارات البحث الحالية.
                                        ...
                                        filteredRows.map((row) => (
                                            <tr key={row.id} className="hover:bg-slate-50 transition dark:hover:bg-slate-800/50">
                                                {activeTable.columns_definition.map((col) => {
                                                    const isEditing =
                                                        hasPermission(userRole, "canEditRow") &&
                                                        editingCell?.rowId === row.id &&
                                                                ...
                                                                <div className="p-1 rounded min-h-[20px] flex items-center gap-1.5 flex-wrap">
                                                                    {col.type === 'relation' && (
                                                                        <div className="flex flex-wrap gap-1">
                                                                            {Array.isArray(cellValue) && cellValue.length > 0 ? (
                                                                                cellValue.map((id) => (
                                                                                    <span key={id} className="inline-flex items-center gap-1 bg-blue-50 dark:bg-cyan-950/40 border border-blue-200 dark:border-cyan-800/30 text-blue-600 dark:text-cyan-400 px-2 py-0.5 rounded text-[10px] underline">
                                                                                        <Link className="w-2.5 h-2.5" />
                                                                                        {getRelationDisplayValue(col, id)}
                                                                                    </span>
                                                                                ))
                                                                            ) : <span className="text-slate-400 italic text-[11px]">غير مرتبط</span>}
                                                                        </div>
                                                                    )}

                                                                    {col.type === 'attachment' && (
                                                                        cellValue ? (
                                                                            <div className="flex items-center gap-2 bg-slate-100 dark:bg-slate-950/60 p-1 px-2 rounded-lg border border-gray-200 dark:border-slate-800">
                                                                                <a href={cellValue} target="_blank" rel="noreferrer" className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-bold underline hover:text-emerald-500">
                                                                                    <Paperclip className="w-3 h-3" />
                                                                                    <span className="max-w-[120px] truncate">{getFileNameFromUrl(cellValue)}</span>
                                                                                    <ExternalLink className="w-2.5 h-2.5" />
                                                                                </a>
                                                                                ...
                                                                                {hasPermission("write") &&
                                                                                    <button
                                                                                        ...
                                                                                        className="text-red-500 hover:text-red-600 dark:text-red-400 dark:hover:text-red-300 hover:bg-red-50 dark:hover:bg-red-950/50 p-0.5 rounded transition"
                                                                                        title="حذف المستند نهائياً"
                                                                                    >
                                                                                        <Trash2 className="w-3 h-3" />
                                                                                    </button>
                                                                        ...
                                                                        ) : <span className="text-slate-400 italic text-[11px]">لا يوجد ملف</span>
                                                                    )}

                                                                    {col.type === 'dropdown' && cellValue && <span className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-950 text-blue-600 dark:text-blue-400 border border-gray-200 dark:border-slate-800 text-[10px]">{cellValue}</span>}
                                                                    {col.type === "system_pages" ? (
                                                                        (() => {
                                                                            let permissions = {};
                                                                            ...
                                                                            return (
                                                                                <div className="flex flex-wrap gap-1">
                                                                                    {Object.entries(permissions).map(([page, permission]) => (
                                                                                        <span
                                                                                            key={page}
                                                                                            className="px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 text-[10px]"
                                                                                        >
                                                                                            {page}: {permission}
                                                                                        </span>
                                                                                    ))}
                                                ...
                                                <td className="p-4 text-left flex items-center justify-end gap-2">
                                                    <GlobalDocumentGenerator
                                                        ...
                                                        selectedRow={row}
                                                        triggerButton={
                                                            <button className="px-2.5 py-1.5 bg-blue-50 dark:bg-blue-950/40 hover:bg-blue-600 dark:hover:bg-blue-500 text-blue-600 dark:text-blue-400 hover:text-white dark:hover:text-white rounded-xl text-[11px] font-bold transition flex items-center gap-1 border border-blue-200 dark:border-blue-500/20">
                                                                <Wand2 className="w-3 h-3" />
                                                                توليد مستند
                                                            </button>
                                                        }
                                                    ...
                                                    {hasPermission(userRole, "canEditRow") && (
                                                        <div className="flex gap-2">
                                                            <button onClick={() => openEditRowModal(row)} className="text-[10px] text-blue-600 dark:text-blue-400 hover:underline px-2 py-1 transition-colors">تعديل</button>
                                                            <button onClick={() => handleDeleteRow(row.id)} className="text-[10px] text-red-500 dark:text-red-400 hover:underline px-2 py-1 transition-colors">حذف</button>
                                                        </div>
                                                    )}
                                                </td>
                                            </tr>
                    ...
                    {viewMode === 'grid' && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {filteredRows.map((row) => (
                                <div
                                    key={row.id}
                                    className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl p-5 shadow-sm dark:shadow-slate-950/20 flex flex-col justify-between space-y-4 transition"
                                >
                                    <div className="space-y-2.5">
                                        {activeTable.columns_definition.map((col, idx) => (
                                            <div
                                                ...
                                                className={
                                                    idx === 0
                                                        ? "border-b border-slate-200 dark:border-slate-700 pb-2 mb-2"
                                                        : ""
                                                }
                                            >
                                                ...
                                                <span
                                                    className={`text-xs font-semibold flex items-center gap-1.5 flex-wrap ${idx === 0
                                                        ? "text-blue-600 dark:text-blue-400 text-sm font-black"
                                                        : "text-slate-700 dark:text-slate-300"
                                                        }`}
                                                >
                                                    {col.type === "relation" && (
                                                        <div className="flex flex-wrap gap-1">
                                                            {Array.isArray(row.cells_data[col.id]) &&
                                                                row.cells_data[col.id].length > 0 ? (
                                                                row.cells_data[col.id].map((relatedId) => (
                                                                    <span
                                                                        key={relatedId}
                                                                        className="inline-flex items-center gap-1 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 text-blue-700 dark:text-blue-300 px-2 py-0.5 rounded text-[11px]"
                                                                    >
                                                                        <Link className="w-2.5 h-2.5" />
                                                                        {getRelationDisplayValue(col, relatedId) || "-"}
                                                                    </span>
                                                    ...
                                                    {col.type === "attachment" && (
                                                        row.cells_data[col.id] ? (
                                                            <a
                                                                href={row.cells_data[col.id]}
                                                                target="_blank"
                                                                rel="noreferrer"
                                                                className="text-emerald-600 dark:text-emerald-400 underline flex items-center gap-1"
                                                            >
                                                                <Paperclip className="w-3 h-3" />
                                                                {getFileNameFromUrl(row.cells_data[col.id])}
                                                            </a>
                                                        ) : (
                                                            "-"
                                                        )
                                    ...
                                    <div className="flex justify-end gap-2 border-t border-slate-200 dark:border-slate-700 pt-3">
                                        {hasPermission(userRole, "canGenerateDocument") && (
                                            <GlobalDocumentGenerator
                                                ...
                                                selectedRow={row}
                                                triggerButton={
                                                    <button className="px-2.5 py-1.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-[11px] font-bold transition flex items-center gap-1 shadow-sm">
                                                        <Wand2 className="w-3 h-3" />
                                                        توليد مستند
                                                    </button>
                                                }
                                        ...
                                        <div className="flex gap-2">
                                            {hasPermission(userRole, "canEditRow") && (
                                                <button
                                                    onClick={() => openEditRowModal(row)}
                                                    className="text-[10px] text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 px-2 py-1 transition-colors"
                                                >
                                                    تعديل
                                                </button>
                                            )}
                                            ...
                                            {hasPermission(userRole, "canDeleteRow") && (
                                                <button
                                                    onClick={() => handleDeleteRow(row.id)}
                                                    className="text-[10px] text-red-600 hover:text-red-700 dark:text-red-400 dark:hover:text-red-300 px-2 py-1 transition-colors"
                                                >
                                                    حذف
                                                </button>
                                            )}
                    ...
                    {viewMode === 'list' && (
                        <div className="space-y-3">
                            {filteredRows.length === 0 ? (
                                <div className="p-8 text-center text-slate-500 dark:text-slate-400 bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 italic">
                                    لا توجد مستندات أو سجلات تطابق البحث الحالي.
                                </div>
                            ) : (
                                filteredRows.map((row) => {
                                    const primaryCol = activeTable.columns_definition[0];
                                    ...
                                    return (
                                        <div
                                            key={row.id}
                                            className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white dark:bg-slate-900 hover:bg-slate-50 dark:hover:bg-slate-800 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm dark:shadow-slate-950/20 transition-all gap-4"
                                        >
                                            <div className="flex items-center gap-3">
                                                <div className="bg-blue-50 dark:bg-slate-800 p-2.5 rounded-lg border border-blue-200 dark:border-slate-700 text-blue-600 dark:text-blue-400">
                                                    <FileText className="w-4 h-4" />
                                                </div>
                                                ...
                                                <div>
                                                    ...
                                                    <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1">
                                                        {activeTable.columns_definition.slice(1, 4).map((col) => (
                                                            <span
                                                                key={col.id}
                                                                className="text-[10px] text-slate-500 dark:text-slate-400 font-medium flex items-center gap-1"
                                                            >
                                                                {col.name}:{' '}
                                            ...
                                            {/* 🛠️ أزرار خيارات نمط القائمة */}
                                            <div className="flex items-center gap-2 w-full sm:w-auto justify-end border-t sm:border-0 border-slate-200 dark:border-slate-700 pt-2 sm:pt-0">
                    ...
                    {isRowModalOpen && activeTable && (
                        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                            <div className="bg-white border text-slate-800 border-gray-200 rounded-2xl w-full max-w-xl p-6 shadow-2xl space-y-4 dark:bg-slate-900 dark:text-slate-100 dark:border-slate-700" dir="rtl">
                                {/* Header Section */}
                                <div className="flex justify-between items-center border-b border-gray-200 dark:border-slate-800 pb-3">
                                    <h3 className="text-sm font-black text-blue-600 dark:text-blue-500">
                                        {!hasPermission(userRole, "canEditRow")
                                            ? "🔍 استعراض بيانات السجل القانوني الحالي (قراءة فقط)"
                                            : editingRow
                                                ? "📝 تعديل بيانات السجل القانوني الحالي"
                                ...
                                <div className="space-y-4 max-h-[60vh] overflow-y-auto pl-1 pr-1">
                                    {activeTable.columns_definition.map((col) => (
                                        <div key={col.id} className="text-right">
                                            ...
                                            {col.type === 'dropdown' ? (
                                                <select
                                                    ...
                                                    onChange={(e) => setRowData({ ...rowData, [col.id]: e.target.value })}
                                                    disabled={!hasPermission(userRole, "canEditRow")}
                                                    className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 rounded-xl p-2.5 text-xs text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:focus:border-blue-500 dark:focus:ring-blue-500/20 shadow-sm transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                                                >
                                                    <option value="">إختر خياراً...</option>
                                                    {col.options?.map((opt, i) => (
                                                        <option key={i} value={opt}>
                                                ...
                                                <div className="space-y-2">
                                                    <div
                                                        className={`w-full bg-white dark:bg-slate-800 border border-dashed border-gray-300 dark:border-slate-700 rounded-xl p-4 text-center relative shadow-sm transition-all ${hasPermission(userRole, "canEditRow")
                                                        ...
                                                        {uploadingField === col.id ? (
                                                            ...
                                                            <div className="flex flex-col items-center gap-1.5">
                                                                <label
                                                                    ...
                                                                    className={`text-xs text-slate-500 dark:text-slate-400 flex flex-col items-center gap-1.5 ${hasPermission(userRole, "canEditRow")
                                                                        ? "cursor-pointer"
                                                                        : "cursor-not-allowed"
                                                                        }`}
                                                                >
                                                                    ...
                                                                    {rowData[col.id] ? (
                                                                        <span className="text-blue-600 dark:text-blue-400 font-bold max-w-full truncate block px-4">
                                                                            ✓ المستند المرفوع: {getFileNameFromUrl(rowData[col.id])}
                                                                        </span>
                                                                    ) : !hasPermission(userRole, "canEditRow") ? (
                                                                        "لا يوجد مستند مرفق حالياً"
                                                                ...
                                                                {rowData[col.id] && (
                                                                    <div className="flex items-center gap-4 mt-2 border-t border-gray-200 dark:border-slate-700 pt-2 w-full justify-center">
                                                                        <button
                                                                            type="button"
                                                                            onClick={() => handleViewDocument(rowData[col.id])}
                                                                            className="text-[11px] text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 underline cursor-pointer font-bold transition-all"
                                                                        >
                                                                            معاينة هذا المستند ↗
                                                                        </button>
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            className="text-[11px] text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 cursor-pointer font-bold flex items-center gap-1 transition-all"
                                                                        >
                                                                            🗂️ عرض أرشيف الموكل الشامل
                                                                        </button>
                                                                    </div>
                                                ...
                                                <div className="space-y-1">
                                                    <select
                                                        ...
                                                        className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 rounded-xl p-2.5 text-xs text-slate-800 dark:text-slate-100 font-bold min-h-[100px] focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:focus:border-blue-500 dark:focus:ring-blue-500/20 shadow-sm transition-all disabled:opacity-60 disabled:cursor-not-allowed"
                                                    >
                                                        {(relationRowsMap[getRelationTableId(col)] || []).map((rRow) => (
                                                            <option
                                                                key={rRow.id}
                                                ...
                                                <div className="p-3 bg-white dark:bg-slate-800 rounded-xl border border-gray-300 dark:border-slate-700 shadow-sm space-y-3">
                                                    {(() => {
                                                        let pagesPermissions = {};
                                                        try {
                                                            pagesPermissions =
                                                        ...
                                                        return SYSTEM_PAGES.map((page) => {
                                                            const currentPermission = pagesPermissions[page.id] || "no_access";
                                                            ...
                                                            return (
                                                                <div
                                                                    key={page.id}
                                                                    className="flex justify-between items-center bg-white dark:bg-slate-900 p-2 rounded-lg border border-gray-200 dark:border-slate-800 text-xs"
                                                                >
                                                                    <div>
                                                                    ...
                                                                    <div className="flex gap-1 text-[11px]">
                                                                        <button
                                                                            ...
                                                                            className={
                                                                                currentPermission === "no_access"
                                                                                    ? "px-2 py-1 bg-red-600 text-white rounded font-bold transition-all shadow-sm"
                                                                                    : "px-2 py-1 bg-gray-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded hover:bg-gray-200 dark:hover:bg-slate-700 transition-all"
                                                                            }
                                                                        >
                                                                            منع
                                                                        </button>
                                                                        ...
                                                                        <button
                                                                            ...
                                                                            className={
                                                                                currentPermission === "read"
                                                                                    ? "px-2 py-1 bg-blue-600 dark:bg-blue-500 text-white rounded font-bold transition-all shadow-sm"
                                                                                    : "px-2 py-1 bg-gray-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded hover:bg-gray-200 dark:hover:bg-slate-700 transition-all"
                                                                            }
                                                                        >
                                                                            قراءة
                                                        ...
                                                        {systemTables && systemTables.length > 0 ? (
                                                            systemTables
                                                                ...
                                                                .map((table) => {
                                                                    ...
                                                                    return (
                                                                        <div key={table.id} className="flex flex-col sm:flex-row justify-between items-start sm:items-center bg-white dark:bg-slate-900 p-2 rounded-lg border border-gray-200 dark:border-slate-700 gap-2">
                                                                            <span className="text-xs text-slate-800 dark:text-slate-100 font-medium flex items-center gap-1.5">
                                                                                📊 {table.name || 'جدول غير مسمى'}
                                                                            </span>
                                                                            <div className={`flex gap-1 bg-gray-100 dark:bg-slate-800 p-0.5 rounded border border-gray-300 dark:border-slate-700 self-end sm:self-auto ${!hasPermission(userRole, "canEditRow") ? 'pointer-events-none opacity-80' : ''}`}>
                                                                                <button
                                                                                    type="button"
                                                                                    disabled={!hasPermission(userRole, "canEditRow")}
                                                                                    onClick={() => updatePermissionState('no_access')}
                                                                                    className={`px-2 py-1 text-[10px] rounded transition-all font-bold ${hasPermission(userRole, "canEditRow") ? 'cursor-pointer' : ''} ${currentPermission === 'no_access' ? 'bg-red-600 text-white border border-red-500/30' : 'bg-gray-100 text-slate-600 dark:text-slate-400 hover:bg-gray-200 dark:hover:bg-slate-600'}`}
                                                                                >
                                                                                    حجب
                                                                                </button>
                                                                                <button
                                                                                    ...
                                                                                    disabled={!hasPermission(userRole, "canEditRow")}
                                                                                    onClick={() => updatePermissionState('read_only')}
                                                                                    className={`px-2 py-1 text-[10px] rounded transition-all font-bold ${hasPermission(userRole, "canEditRow") ? 'cursor-pointer' : ''} ${currentPermission === 'read_only' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/30' : 'bg-gray-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 hover:bg-gray-200 dark:hover:bg-slate-600'}`}
                                                                                >
                                                                                    قراءة
                                                                                </button>
                                                                                <button
                                                                                    ...
                                                                                    disabled={!hasPermission(userRole, "canEditRow")}
                                                                                    onClick={() => updatePermissionState('read_write')}
                                                                                    className={`px-2 py-1 text-[10px] rounded transition-all font-bold ${hasPermission(userRole, "canEditRow") ? 'cursor-pointer' : ''} ${currentPermission === 'read_write' ? 'bg-green-600 text-white border border-emerald-500/30' : 'bg-gray-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 hover:bg-gray-200 dark:hover:bg-slate-600'}`}
                                                                                >
                                                                                    كاملة
                                                                                </button>
                                                                            </div>
                                ...
                                <div className="flex items-center justify-end gap-3 border-t border-gray-200 dark:border-slate-800 pt-4 mt-2">
                                    <button
                                        type="button"
                                        onClick={() => setIsRowModalOpen(false)}
                                    ...
                                    {hasPermission(userRole, "canEditRow") && (
                                        <button
                                            ...
                                            className="px-5 py-2 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 dark:bg-blue-500 dark:hover:bg-blue-600 rounded-xl shadow-md shadow-blue-500/10 transition-all flex items-center gap-1.5"
                                        >
                                            {editingRow ? "حفظ التغييرات" : "إضافة السجل"}
                                        </button>
                                    )}
                                </div>
                            </div>
                    ...
                    {isSettingsModalOpen && activeTable && hasPermission(userRole, "canViewSettings") && (
                        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4 transition-all">
                            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl w-full max-w-xl p-6 shadow-2xl space-y-4" dir="rtl">
                                <div className="flex justify-between items-center border-b border-gray-200 dark:border-slate-800 pb-3">
                                    <h3 className="text-sm font-black text-blue-600 dark:text-blue-400">⚙️ باني مخطط الحقول المتقدم (Dynamic Schema Builder)</h3>
                                    <button
                                        onClick={() => setIsSettingsModalOpen(false)}
                                        className="text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-400 transition"
                                    >
                                        <X className="w-4 h-4" />
                                    </button>
                                </div>
                                ...
                                {/* 🔒 أزرار وحقول إضافة حقل جديد تظهر فقط في حال صلاحية 'write' */}
                                <div className="bg-slate-50 dark:bg-slate-950 p-4 rounded-xl border border-gray-200 dark:border-slate-800 space-y-3">
                                    <span className="block text-[10px] text-blue-600 dark:text-blue-400 font-bold">➕ إضافة حقل ذكي جديد وتحديد المنطق:</span>
                                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                        <input
                                            type="text"
                                            placeholder="اسم الحقل (مثال: صلاحيات الوصول)..."
                                            value={newColumnName}
                                            onChange={(e) => setNewColumnName(e.target.value)}
                                            className="bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 rounded-xl p-2 text-xs text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 shadow-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:focus:border-blue-500 dark:focus:ring-blue-500/20 transition-all"
                                        />
                                        <select
                                            value={newColumnType}
                                            onChange={(e) => setNewColumnType(e.target.value)}
                                        ...
                                        <button
                                            onClick={handleAddColumnStructure}
                                            className="bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 text-white rounded-xl text-xs font-black shadow-sm transition-all cursor-pointer py-2"
                                        >
                                            إضافة الحقل للمخطط
                                        </button>
                                    </div>
                                    ...
                                    {fieldSupportsOptions(newColumnType) && (
                                        <div className="p-3 bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 space-y-2 text-right shadow-sm">
                                            <label className="block text-[10px] text-slate-500 dark:text-slate-400 font-bold">خيارات القائمة المنسدلة:</label>
                                            <div className="flex gap-2">
                                                <input
                                                    type="text"
                                                    placeholder="مثال: قيد التنفيذ، منتهية..."
                                                    value={newOptionInput}
                                                    onChange={(e) => setNewOptionInput(e.target.value)}
                                                    className="flex-1 bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 rounded-xl p-1.5 text-xs text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 shadow-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:focus:border-blue-500 dark:focus:ring-blue-500/20 transition-all"
                                                />
                                                <button
                                                    onClick={handleAddDropdownOption}
                                                    className="bg-slate-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 px-3 text-xs rounded-xl text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 transition-all cursor-pointer"
                                                >
                                                    <ListPlus className="w-3.5 h-3.5" />
                                                </button>
                                            </div>
                                            <div className="flex flex-wrap gap-1.5 pt-1 justify-start">
                                                {dropdownOptions.map((opt, idx) => (
                                                    <span key={idx} className="bg-slate-100 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 px-2 py-0.5 rounded-lg text-[10px]">
                                                        {opt}
                                                    </span>
                                                ))}
                                            </div>
                                    ...
                                    {(fieldSupportsRelation(newColumnType)) && (
                                        <div className="mt-3 animate-in fade-in duration-200 p-3 bg-white dark:bg-slate-900/50 rounded-xl border border-gray-200 dark:border-slate-800/60 text-right shadow-sm">
                                            <label className="block text-xs font-bold text-cyan-600 dark:text-cyan-400 mb-1.5">
                                                {newColumnType === 'user_account' ? '👤 حدد الجدول الرئيسي لمنح صلاحيات الحساب عليه:' : '🔗 اختر الجدول المرتبط المستهدف (Many-to-Many):'}
                                            </label>
                                            <select
                                                value={selectedRelationTableId}
                                                onChange={(e) => setSelectedRelationTableId(e.target.value)}
                                                className="w-full bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-700 rounded-xl p-2 text-xs text-slate-800 dark:text-slate-100 placeholder:text-slate-400 dark:placeholder:text-slate-500 shadow-sm focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 dark:focus:border-blue-500 dark:focus:ring-blue-500/20 transition-all"
                                            >
                                                <option value="">-- اختر الجدول المطلوب من النظام --</option>
                                                {systemTables && systemTables.map((table) => (
                                                    <option key={table.id} value={table.id}>
                                ...
                                <div className="space-y-2 text-right">
                                    <span className="block text-[10px] text-slate-500 dark:text-slate-400 font-bold">مخطط الحقول الحالي للجدول:</span>
                                    <div className="max-h-[30vh] overflow-y-auto space-y-2 pl-1">
                                        {manageColumns.map((col, index) => (
                                            <div
                                                key={col.id}
                                                ...
                                                onDragOver={(e) => handleDragOver(e, index)}
                                                onDragEnd={handleDragEnd}
                                                className="flex justify-between items-center bg-slate-50 dark:bg-slate-950 p-3 rounded-xl border border-gray-200 dark:border-slate-800 gap-4 transition-all"
                                            >
                                                {/* 🔒 زر حذف حقل يظهر فقط للمحامي/الصلاحية العليا للمخطط */}
                                                {hasPermission(userRole, "canEditRow") && (
                                                    <button
                                                        onClick={() => handleRemoveColumnStructure(col.id)}
                                                        className="text-red-500 hover:bg-red-50 dark:hover:bg-red-950/20 p-2 rounded-lg transition-colors cursor-pointer"
                                                    >
                                                        <Trash2 className="w-3.5 h-3.5" />
                                                    </button>
                                                )}
                                                ...
                                                <div className="flex items-center gap-3 text-xs w-full justify-end">
                                                    <div className="flex items-center gap-2 flex-wrap justify-end">
                                                        {col.type === 'user_account' && (
                                                            <span className="text-[9px] text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-1.5 py-0.5 rounded border border-emerald-200 dark:border-emerald-900/30 font-bold flex items-center gap-0.5">
                                                                👤 مصفوفة صلاحيات {col.relationTableId ? `[جدول مرتبط]` : ''}
                                                            </span>
                                                        )}
                                                        {col.type === 'staff_password' && (
                                                            <span className="text-[9px] text-rose-600 dark:text-rose-400 bg-rose-50 dark:bg-rose-950/40 px-1.5 py-0.5 rounded border border-rose-200 dark:border-rose-900/30 font-bold flex items-center gap-0.5">
                                                                🔒 مشفر سري
                                                            </span>
                                                        )}
                                                        {col.type === 'staff_email' && (
                                                            <span className="text-[9px] text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-950/40 px-1.5 py-0.5 rounded border border-blue-200 dark:border-blue-900/30 font-bold flex items-center gap-0.5">
                                                                📧 بريد رسمي
                                                            </span>
                                                        )}
                                                        {col.type === 'relation' && <span className="text-[9px] text-cyan-600 dark:text-cyan-400 font-bold">(متعدد 🔗)</span>}
                                                        {col.type === 'dropdown' && <span className="text-[9px] text-blue-600 dark:text-blue-400 font-bold">({col.options?.length} خيارات)</span>}

                                                        <span className="px-2 py-0.5 bg-slate-200 dark:bg-slate-900 text-slate-600 dark:text-slate-400 rounded text-[10px]">{col.type}</span>
                                                        <span className="text-slate-800 dark:text-slate-200 font-bold">{col.name}</span>
                                                    </div>
                                                    {hasPermission(userRole, "canEditRow") && <div className="cursor-grab text-slate-400 dark:text-slate-600 font-bold select-none">☰</div>}
                                                </div>
                                            </div>
                                        ))}
                                    </div>
                                ...
                                <div className="flex justify-end gap-2 border-t border-gray-200 dark:border-slate-800 pt-4">
                                    ...
                                    {hasPermission(userRole, "canEditRow") && (
                                        <button
                                            onClick={handleSaveChangesStructure}
                                            className="px-5 py-2 bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 text-white dark:text-slate-950 text-xs font-black rounded-xl flex items-center gap-1.5 transition-all shadow-sm cursor-pointer"
                                        >
                                            <Save className="w-4 h-4" /> حفظ التغييرات الهيكلية
                                        </button>
                                    )}
                                    ...
                                    <GlobalDocumentGenerator
                                        ...
                                        filteredRows={filteredRows}
                                        triggerButton={
                                            <button className="bg-blue-600 dark:bg-blue-500 hover:bg-blue-700 dark:hover:bg-blue-600 text-white hover:text-white dark:hover:text-slate-950 px-4 py-2 rounded-xl text-xs font-bold transition-all flex items-center gap-1.5 shadow-sm cursor-pointer">
                                                <Wand2 className="w-4 h-4" />
                                                توليد مستند شامل للجدول
                                            </button>
                                        }
                                    ...
                                    <button
                                        onClick={() => setIsSettingsModalOpen(false)}
                                        className="bg-slate-100 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 px-4 py-2 rounded-xl text-xs text-slate-600 dark:text-slate-400 hover:bg-slate-200 dark:hover:bg-slate-900 transition-all cursor-pointer"
                                    >
                                        إلغاء
                                    </button>
                                </div>
                    ...
                    {/* 📂 مودال استعراض أرشيف ملف الموكل الشامل */}
                    {isClientDocsModalOpen && editingRow && (
                        <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-[60] p-4 animate-in fade-in duration-200 transition-all">
                            <div className="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-2xl w-full max-w-4xl p-6 shadow-2xl space-y-4 max-h-[85vh] overflow-y-auto relative" dir="rtl">
                                <button
                                    onClick={() => setIsClientDocsModalOpen(false)}
                                    className="absolute top-4 left-4 text-slate-400 hover:text-slate-600 dark:text-slate-500 dark:hover:text-slate-300 bg-slate-100 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 p-2 rounded-xl transition cursor-pointer z-10"
                                >
                                    <X className="w-4 h-4" />
                                </button>
                                ...
                                <div className="flex justify-end border-t border-gray-200 dark:border-slate-800 pt-4">
                                    <button
                                        onClick={() => setIsClientDocsModalOpen(false)}
                                        className="bg-slate-100 dark:bg-slate-950 border border-gray-200 dark:border-slate-800 hover:bg-slate-200 dark:hover:bg-slate-900 px-6 py-2 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 transition cursor-pointer shadow-sm"
                                    >
                                        إغلاق الأرشيف المعروض
                                    </button>
                                </div>
