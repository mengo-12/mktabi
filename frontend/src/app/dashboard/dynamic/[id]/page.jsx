...
export default function DynamicSectionPage() {
    ...
    // العمل على الصفوف
    const [editingRow, setEditingRow] = useState(null); // إذا كان يحمل كائن يعني وضع التعديل، null يعني وضع إضافة
    ...
    const loadSectionContent = async () => {
        try {
            ...
            if (tablesData.length > 0) {
                // المحافظة على الجدول النشط الحالي إن وجد بعد التحديث، وإلا اختر الأول
    ...
    const handleSaveRow = async () => {
        try {
            if (editingRow) {
                // 🎯 الإصلاح: تغليف البيانات داخل كائن cells_data ليتوافق مع الباك إند
    ...
    const handleDeleteRow = async (rowId) => {
        if (confirm("هل أنت متأكد من حذف هذا السجل نهائياً؟")) {
            try {
                ...
                // 🔥 الإصلاح اللحظي: تحديث القائمة بعد الحذف مباشرة
                if (activeTable?.id) {
                    await loadTableRows(activeTable.id);
                }
            } catch (error) {
    ...
    const handleSaveChangesStructure = async () => {
        try {
            ...
            loadSectionContent(); // إعادة جلب البيانات والهيكل الجديد
        } catch (error) {
    ...
    return (
        <div className="md:mr-64 p-6 bg-[#0B0F19] min-h-screen text-slate-100 font-sans min-w-0" dir="rtl">
            ...
            {activeTable && (
                <div>
                    {/* شريط التحكم الرئيسي للجدول النشط */}
                    ...
                    {/* 🖥️ 1. نمط شبكة إكسل (Table View) مع عمليات التحكم */}
                    {viewMode === 'table' && (
                                ...
                                <tbody className="text-xs text-slate-300 divide-y divide-slate-800/50">
                                    {rows.map((row) => (
                                        <tr key={row.id} className="hover:bg-slate-850/40 transition">
                                            {activeTable.columns_definition.map((col) => (
                                                <td key={col.id} className="p-4 font-medium">
                                                    {row.cells_data[col.name] || '-'}
                                                </td>
                                            ))}
                                            {/* أزرار تعديل وحذف الصف */}
                                            <td className="p-4 text-left flex items-center justify-end gap-2">
                                                <button onClick={() => openEditRowModal(row)} className="p-1.5 bg-slate-950 hover:bg-slate-800 rounded-lg text-blue-400 hover:text-blue-300 transition" title="تعديل الصف"><Edit3 className="w-3.5 h-3.5" /></button>
                                                <button onClick={() => handleDeleteRow(row.id)} className="p-1.5 bg-slate-950 hover:bg-red-950 rounded-lg text-red-400 hover:text-red-300 transition" title="حذف الصف"><Trash2 className="w-3.5 h-3.5" /></button>
                                            </td>
                                        </tr>
                                    ))}
                                    {rows.length === 0 && (
                                        <tr>
                                            <td colSpan={activeTable.columns_definition.length + 1} className="text-center p-8 text-slate-500">لا توجد سجلات مدخلة حتى الآن.</td>
                                        </tr>
                                    )}
                                </tbody>
                            </table>
                    ...
                    {/* 🎴 2. نمط بطاقات فخمة مع خيارات التعديل والحذف */}
                    {viewMode === 'grid' && (
                    ...
                    {/* 📝 3. نمط القائمة الطولية مع خيارات التعديل والحذف */}
                    {viewMode === 'list' && (
                                    ...
                                    <div className="flex flex-wrap gap-6 items-center text-xs">
                                        {activeTable.columns_definition.map((col) => (
                                            <div key={col.id}>
                                                ...
                                                <span className="text-slate-300 font-medium">{row.cells_data[col.name] || '-'}</span>
                                            </div>
                                        ))}
                                    </div>
            ...
            {/* 📥 النافذة المنبثقة: إضافة أو تعديل صف (سجل بيانات) */}
                            ...
                            {activeTable.columns_definition.map((col) => (
                                <div key={col.id}>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) => setRowData({ ...rowData, [col.name]: e.target.value })}
            ...
            {/* ⚙️ النافذة المنبثقة: إدارة أعمدة الجدول وهيكل الحقول الحالية */}
            {isSettingsModalOpen && activeTable && (
                <div className="fixed inset-0 bg-slate-950/80 backdrop-blur-sm flex items-center justify-center z-50 p-4">
                    <div className="bg-slate-900 border border-slate-800 rounded-2xl w-full max-w-xl p-6 shadow-2xl space-y-4">
                        ...
                        {/* فورم سريع لإضافة عمود جديد للهيكل المستهدف */}
                        ...
                        {/* قائمة الأعمدة الحالية والمضافة مع خيار حذف أي منها */}
                        <div className="space-y-2">
                            <span className="block text-[10px] text-slate-400 font-bold">الأعمدة المعتمدة حالياً للجدول:</span>
                            <div className="max-h-[30vh] overflow-y-auto space-y-1.5 pr-1">
                                {manageColumns.map((col) => (
                                    <div key={col.id} className="flex justify-between items-center bg-slate-950/60 p-2.5 rounded-lg border border-slate-800/80">
                                        ...
                                        <button
                                            ...
                                            className="text-red-500 hover:text-red-400 p-1 rounded hover:bg-red-950/20 transition"
                                            title="حذف العمود"
                                        >
                                ...
                                {manageColumns.length === 0 && (
                                    <p className="text-center text-xs text-slate-600 py-4">الجدول فارغ من الأعمدة تماماً، قم بإضافة حقل واحد على الأقل.</p>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-2 border-t border-slate-800 pt-4">
                            ...
                            <button
                                ...
                                disabled={manageColumns.length === 0}
                                className="bg-blue-600 hover:bg-blue-500 disabled:bg-slate-800 text-slate-100 px-5 py-2 rounded-xl text-xs font-bold flex items-center gap-1 transition"
                            >
                                <Save className="w-4 h-4" /> حفظ وتطبيق الهيكل الجديد
                            </button>
                        </div>
                    </div>
                </div>
